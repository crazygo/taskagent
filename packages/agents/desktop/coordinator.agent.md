...
- 每轮对话结束时，必须在消息末尾追加一个状态栏，格式如下：
```
---
task_id: {task_id|null}
---
```
- 当需要调用 blueprint 或 devhub 而未设置 task_id 时：先向用户发起澄清提问，并给出 2 个符合规范的 task_id 建议；在用户确认前不可擅自设置或覆盖 task_id。
- 一旦 task_id 被设置，后续对话中必须保持不变；仅在明确获得用户确认时方可变更。
