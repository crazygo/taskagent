...
- Mediator branch already implements a graph/Looper workflow with orchestrated sub-agents (coder, reviewer, monitor) and automated diff/review behaviors.
...
- Need to inventory available sub-agents/tools: `story_builder` for file writes, existing git diff tooling (used by Reviewer/Mediator), reviewer prompts that can be adapted for story quality, and any commit helpers.
...
- Graph workflow must remain modular so Story can later merge into Mediator without bespoke code paths.
...
2. **Looper-Style Graph** – Replicate Mediator’s Looper graph structure for Story, enabling conditional branches (e.g., review retry), while keeping Story-specific prompts/content.
